...
//            K spilled_element = binHeap[last];
//            spilledPQ.give(spilled_element);
            last--;
    ...
    void swimUp(int k) {
        int i = k;
        while (i > first && unordered(parent_idx(i), i)) {
            swap(i, parent_idx(i));
            i = parent_idx(i);
        }
    }
    ...
    private int parent_idx_4ary(int k) {
        return (k+2)/4;
    }

    private int firstChild_4ary(int k) {
        return 4*k-2;
    }

    private int doHeapify_4ary(int k, BiPredicate<Integer, Integer> p) {
        int i = k;
        // 先計算第一個子節點
        int c = firstChild_4ary(i);

        // 在確保子節點 c 有效(不超過 last)的前提下進行迴圈
        while (c <= last) {
            // 找出 i 的 4個子節點中最「優先」的(若 max-heap 就是最大的)
            // 先假設 j=c
            int j = c;
            // 其餘三個 child => c+1, c+2, c+3
            // 需要先檢查是否在範圍內 ( j <= last )
            for (int offset = 1; offset < 4; offset++) {
                int nxt = c + offset;
                if (nxt <= last && unordered(j, nxt)) {
                    j = nxt; // j 指向更「優先」的子節點
                }
            }

            // 如果 parent i 與 j 的關係符合 p.test(i, j)，表示不需要再下沉
            if (p.test(i, j)) break;
    ...
    void sink_4ary(int k) {
        doHeapify_4ary(k, (parent, child) -> !unordered(parent, child));
    }
